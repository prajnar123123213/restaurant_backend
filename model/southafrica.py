...
class Southafrica(db.Model):
    ...
    def update(self):
        ...
        inputs = Southafrica.query.get(self.id)
    ...
    def restore(data):
        for post_data in data:
            ...
            post = Southafrica.query.filter_by(_title=title).first()
            if post:
                post.update(post_data)
            else:
                post = Southafrica(**post_data)
                post.update(post_data)
                post.create()
...
def initSouthafricas():
    ...
    with app.app_context():
        ...
        southafricas = [
            Southafrica(title='Churrascaria Palace', comment='Good food', content={'type': 'announcement'}, user_id=1),
            Southafrica(title='Piatto Farrarmare', comment='Great lunch', content={'type': 'announcement'}, user_id=2),
