...
class Chile(db.Model):
    ...
    def __repr__(self):
        ...
        """
        return f"Chile(id={self.id}, title={self._title}, comment={self._comment}, content={self._content}, user_id={self._user_id})"
    ...
    def update(self):
        ...
        inputs = Chile.query.get(self.id)
    ...
    def restore(data):
        for post_data in data:
            ...
            post = Chile.query.filter_by(_title=title).first()
            if post:
                post.update(post_data)
            else:
                post = Chile(**post_data)
                post.update(post_data)
                post.create()
        
def initChiles():
    """
    The initChile function creates the Post table and adds tester data to the table.
    ...
    with app.app_context():
        ...
        chiles = [
            Chile(title='Jardin Oriental', comment='10/10 would recommend!', content={'type': 'announcement'}, user_id=1),
            Chile(title='Sabor Calderino', comment='I would give my firstborn child to eat here again.', content={'type': 'announcement'}, user_id=2),
        ]
