...
class Brazil(db.Model):
    ...
    def __repr__(self):
        ...
        """
        return f"Brazil(id={self.id}, title={self._title}, comment={self._comment}, content={self._content}, user_id={self._user_id})"
    ...
    def update(self):
        ...
        inputs = Brazil.query.get(self.id)
    ...
    def restore(data):
        for post_data in data:
            ...
            post = Brazil.query.filter_by(_title=title).first()
            if post:
                post.update(post_data)
            else:
                post = Brazil(**post_data)
                post.update(post_data)
                post.create()
        
def initBrazils():
    """
    The initBrazil function creates the Post table and adds tester data to the table.
    ...
    with app.app_context():
        ...
        brazils = [
            Brazil(title='Churrascaria Palace', comment='The food is to die for', content={'type': 'announcement'}, user_id=1),
            Brazil(title='Fogo de Chao', comment='Loved the service quality <3', content={'type': 'announcement'}, user_id=2),
        ]
