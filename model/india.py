...
class India(db.Model):
    ...
    def update(self):
        ...
        inputs = India.query.get(self.id)
    ...
    def restore(data):
        for post_data in data:
            ...
            post = India.query.filter_by(_title=title).first()
            if post:
                post.update(post_data)
            else:
                post = India(**post_data)
                post.update(post_data)
                post.create()
...
def initIndias():
    ...
    with app.app_context():
        ...
        indias = [
            India(title='Added Group and Channel Select', comment='The Home Page has a Section, on this page we can select Group and Channel to allow blog filtering', content={'type': 'announcement'}, user_id=1),
            India(title='JSON content saving through content"field in database', comment='You could add other dialogs to a post that would allow custom data or even storing reference to uploaded images.', content={'type': 'announcement'}, user_id=2),
            India(title='Allows Post by different Users', comment='Different users seeing content is a key concept in social media.', content={'type': 'announcement'}, user_id=3),
        ]
        
        for India in indias:
            try:
                India.create()
                print(f"Record created: {repr(India)}")
            except IntegrityError:
                '''fails with bad or duplicate data'''
                db.session.remove()
                print(f"Records exist, duplicate email, or error: {India._title}")